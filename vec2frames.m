...
%   Author: Kamil Wojcicki, UTD, July 2011
    ...
    % input validation
    if( isempty(vec) || isempty(Nw) || isempty(Ns) ), error( usage ); 
    end
    if( min(size(vec))~=1 ), error( usage ); 
    end
    if( Nw==0 || Ns==0 ), error( usage ); 
    end
...
%     M = floor((L-Nw)/Ns+1);             % number of frames 
    ...
    otherwise
        sprintf('Direction: %s not supported!\n', direction); 

    end


    % divide the input signal into frames using indexing
    sizepad = [Ns+1 - (L - Ns*M),1];
    vecpad = cat(1, vec, zeros(sizepad));
    frames = vecpad( indexes );


    % return if custom analysis windowing was not requested
    if( isempty(window) || ( islogical(window) && ~window ) ), return; 
    end
